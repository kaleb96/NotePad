<template>
    ...
    <div v-if="tabStatus == 1">
        <div :class="filterName + ' upload-image'" :style="{ backgroundImage : `url(${url})`}"></div>
        <div class="filters">
            <FilterBox :url="url" :filter="filter" v-for= "filter in filterData" :key="filter" @click="changeFilterName">
                    <span>{{ filter }}</span>
            </FilterBox>
        </div>
    </div>
